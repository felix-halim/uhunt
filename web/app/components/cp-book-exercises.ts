...
export class CpBookExercisesComponent implements OnChanges {
  ...
  set_chapter(chapter, type) {
    if (type) {
      this._databaseService.set('cpbook_show', type);
      this.show = type;
    }
    if (this.chapter === chapter && !type) {
      this.chapter = -1;
    } else {
      this.chapter = chapter;
    }
    this.refresh();
    this._databaseService.set('cpbook_chapter', this.chapter);
  }
  ...
  switch_edition(ed) {
    this._databaseService.set('cpbook_edition', this.edition = ed);
    this.refresh();
  }
  ...
  private refresh() {
    this._problemService.ready.then(() => {
      ...
      var c = this.cpbooks[this.edition].chapters[this.chapter];
      if (!c) return;
      var sections = [];
